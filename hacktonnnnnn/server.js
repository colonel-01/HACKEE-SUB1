...
async function callOverpass(query) {
  ...
  try {
    ...
    try {
      ...
      // bubble up the mirror error
      throw errMirror;
    }
  }
}

/* ---------- Endpoint: realtime planner ---------- */
/*
  POST /api/plan-trip-realtime
  body: { startPlace (optional), lat (optional), lon (optional), days (optional) }
  Returns: { success, itinerary: [ { level, place, lat, lon, kinds, info, travelMode, weather, cost } ], ... }
*/
app.post("/api/plan-trip-realtime", async (req, res) => {
  console.log("=== /api/plan-trip-realtime HIT ===", new Date().toISOString());
...
out center;`;
    ...
    const places = elements
        ...
        .map((e) => {
            ...
            return {
                ...
                // ⭐ ADDED FIELDS (required for isFamous())
    ...
    // ---------- NEW LOGIC: Distance-based Levels with 4 Famous Places ----------
    ...
    const itinerary = [];
    // pick cycle: level1 -> level2 -> level3
    let i = 0;
    let dayCounter = 1;
    const used = new Set();
